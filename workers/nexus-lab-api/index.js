...
Redirecting to the call… <a style="color:#ededf0" href="${appUrl}">view on Nexus →</a>
    ...
    // Feed / comments / reactions family → routes-feed.mjs.
    {
      const feedRes = await handleFeed(parts, request, env);
      if (feedRes) return feedRes;
    }
    ...
    // ⚠️ REMOVED: a SECOND, unreachable copy of /comments/:id and /reactions/:id lived
    // here. The blocks above match first and return, so ~85 lines never executed. The
    // dead pair had diverged and was arguably the better implementation (randomUUID ids,
    // 201 + the created object). Deleting the dead copy rather than promoting it keeps
    // the live API contract byte-identical — swapping response shapes is a frontend
...
document.getElementById("btn").addEventListener("click",go);
    ...
    if (parts[0] === "intel" && parts[1] === "axis-backtest" && request.method === "GET") {
      ...
      const CACHE = `axisbt:v8:min${min}`; // v8 = BELOW_RANDOM rung (≤5%) in the shared ladder — bumped so no cached verdict mislabels a reliably-worse read · v7 = random-entry baseline per read + side (`random`) and the population `tide` — merged with v6's per-horizon `drift` (a parallel build); bumped so no v6 payload lacking `random` is served · v6 = + drift baseline · v5 = per-side splits + shadow exit · v4 = + basis_dev axes · v3 = `exit` + `exit24h` (one position per market, shared window, oos)
      try { const c = await env.LAB_STORE.get(CACHE); if (c) return json(JSON.parse(c), request); } catch { /* ignore */ }
      const AGENT_KV = env.NEXUS_AGENT || env.LAB_STORE;
      ...
      const out = {
        ...
        ...scorecard,
        note: "Walk-forward event study on self-logged history — forward returns, no lookahead, first/second-half stability. Axes read INSUFFICIENT until the series matures. A read is not an edge until it's PREDICTIVE here. `exit` (on reads a preset trades) grades the read through that preset's own TP/SL/max-hold along the logged candles, one position per market, net of a 3 bps/side taker fee; `exit24h` is the same exit with a 24h hold on the same entry window; `oos` counts only trades entered after 2026-09-25 04:00 UTC. Informational — they do not change the read's verdict. `random` replays every R-graded event at a random hour — same market, same side, same frozen R contract, drawn only from the window the read fired in — 300 seeded runs, pooled and per side; BEATS_RANDOM (≥95% of replays beaten) / LEANS_ABOVE (≥80%) / NOT_DISTINGUISHABLE / BELOW_RANDOM (≤5% — reliably worse than random; not a licence to invert) / TOO_FEW (<5 events), the same ladder as the Lab backtest. It separates 'the signal picked good moments' from 'the window did the work'. `tide` is what a random long vs a random short earned under the same R contract across every recorded market-hour — what a read's side has to beat. (Distinct from each horizon's `drift`, which subtracts the coin's average h-hour move over its whole recorded history, in bps.) Informational — neither changes any verdict.",
      };
    ...
    if (parts[0] === "intel" && parts[1] === "evidence" && request.method === "GET") {
      ...
      const CACHE = `evidence:v3:${axis}:${hold}`; // v3 = BELOW_RANDOM rung in the baseline verdict
      try { const c = await env.LAB_STORE.get(CACHE); if (c) return json(JSON.parse(c), request); } catch { /* ignore */ }
      const COINS = ["BTC", "ETH", "SOL", "XRP", "DOGE", "BNB", "ARB", "AVAX", "LINK", "HYPE", "SUI", "WLD"];
    ...
    if (parts[0] === "agent" && parts[1]) {
      ...
      if (request.method === "PUT" && !parts[2]) {
        ...
        if (config.signalMode && PRO_STRATEGIES.includes(config.signalMode) && !(await walletIsPro(address, env))) {
          return json({ error: "pro_strategy_locked", strategy: config.signalMode, hint: "MOMENTUM and MEAN_REVERSION require Nexus PRO — hold ARCHITECT-tier $NEXUS or subscribe. Free: CONFLUENCE, FUNDING_ONLY, OI_ONLY, BASIS_FADE." }, request, 402);
        }
      ...
      if (request.method === "POST" && parts[2] === "bankr" && parts[3] === "activate") {
        ...
        if (config.signalMode && PRO_STRATEGIES.includes(config.signalMode) && !(await walletIsPro(address, env))) {
          return json({ error: "pro_strategy_locked", strategy: config.signalMode, hint: "MOMENTUM and MEAN_REVERSION require Nexus PRO — hold ARCHITECT-tier $NEXUS or subscribe. Free strategies: CONFLUENCE, FUNDING_ONLY, OI_ONLY, BASIS_FADE." }, request, 402);
        }
        if (config.dcaEnabled && !(await walletIsPro(address, env))) {
          return json({ error: "pro_dca_locked", hint: "DCA / safety-order mode requires Nexus PRO — hold ARCHITECT-tier $NEXUS or subscribe." }, request, 402);
        }
    ...
    {
      const agentsRes = await handleAgents(parts, request, env, ctx);
      if (agentsRes) return agentsRes;
    }
