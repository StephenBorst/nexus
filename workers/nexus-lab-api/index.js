...
document.getElementById("btn").addEventListener("click",go);
    ...
    if (parts[0] === "intel" && parts[1] === "axis-backtest" && request.method === "GET") {
      ...
      const CACHE = `axisbt:v7:min${min}`; // v7 = random-entry baseline per read + side (`random`) and the population `tide` — merged with v6's per-horizon `drift` (a parallel build); bumped so no v6 payload lacking `random` is served · v6 = + drift baseline · v5 = per-side splits + shadow exit · v4 = + basis_dev axes · v3 = `exit` + `exit24h` (one position per market, shared window, oos)
      try { const c = await env.LAB_STORE.get(CACHE); if (c) return json(JSON.parse(c), request); } catch { /* ignore */ }
      const AGENT_KV = env.NEXUS_AGENT || env.LAB_STORE;
