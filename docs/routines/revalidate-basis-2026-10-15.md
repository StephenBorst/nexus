...
- FORWARD PAPER A/B (started Sept 25): two PAPER wallets, identical config (BASIS_FADE + basisConfirm CVD, BTC/ETH/SOL, TP 2.5 / SL 2) differing ONLY in maxHoldHours:
  ...
  - THIRD ARM (breadth, added Sept 25): 0x325da3ed024f533764407524918a847bcb3f95de — PAPER, BASIS_FADE + CVD, 24h
    ...
    BNB DOGE AVAX LINK).
  ⭐ CLEAN START = 2026-09-26T00:04:16Z (epoch ms 1790381056000) for ALL THREE wallets. Use it as the parity `since`
  and as the start of every paper number. History: the first Sept 25 setup was wrong on all three (a leftover
  `takeProfits` scale-out [1%×75%, 2%×25%] overrode TP 2.5, and the third arm had an EMPTY watchlist, which trades
  NOTHING). borst fixed both and re-reset; at 00:04:16Z the configs were verified live: scale-out off, TP 2.5 / SL 2,
  12h / 24h / 24h, BTC/ETH/SOL / BTC/ETH/SOL / all nine, every ledger at 0 trades. Stamps: 24h `paper_reset_at`
  23:56:49Z, third arm 23:09:29Z (no trades before the fix), 12h was re-activated at 00:04–00:05Z with an empty ledger
  and NO stamp (the Lab hid RESET on an empty record; fixed after) — if it now carries a later `paper_reset_at`, use
  the later of the two for that wallet.
  All three ledgers are clean from the start above (the prior CONFLUENCE-era ledgers are archived in the repo, docs/paper-archive/). Read them with the PUBLIC GET https://og.nexustradinglabs.com/agent/<address> (no signature needed): state.paper_agg = lifetime since reset {trades,wins,losses,grossWin,grossLoss,net,firstTradeAt,lastTradeAt}; state.paper_trades = last ≤50 trades (symbol, side, entry_price, exit_price, pnl, exit reason, opened_at, closed_at).
  ...
  LIVE-vs-GRADED PARITY: GET https://og.nexustradinglabs.com/agent/<address>/parity?since=1790381056000 (public, no signature). Pairs every paper entry with the scoreboard event it traded and lists every graded event the agent was free to take but didn't → verdict CLEAN / DRIFT / UNVERIFIABLE / NO_DATA, with counts {entries, events, matched, unmatchedTrades, missedExplained, missedUnexplained}, the matched pairs (lagMin), unmatchedTrades (with reason) and missed.explained (codes HOLDING / BUSY / COOLDOWN / DAILY_TRADES / DAILY_LOSS / OTHER_MARKET) vs missed.unexplained. Always pass since= explicitly (the 12h wallet has no reset stamp).
  Guard: if a wallet's paper_agg.firstTradeAt is before 2026-09-26T00:04:16Z, something traded before the clean start — say so, and count only paper_trades with opened_at on/after that time (paper_agg is then contaminated and must not be used).
...
- RANDOM-ENTRY BASELINE (built Sept 25): the Lab backtest now reports "vs random entries: beat X% of 300 replays" — same markets, trade count, long/short mix and exits, only the entry TIMING random (seeded). BEATS_RANDOM ≥95% · LEANS_ABOVE ≥80% · BELOW_RANDOM ≤5% (reliably WORSE than random — added Sept 25; not a licence to invert) · else NOT_DISTINGUISHABLE; under 5 trades → TOO_FEW_TRADES. This is the test that separates "the signal picks good moments" from "the exits + market drift did the work".
