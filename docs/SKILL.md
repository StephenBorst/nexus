...
**Hold `walletSig` + `walletAddress` in volatile session memory** and reuse them on every authed
call — put them in the JSON **body**, NEVER a query string:
```
POST /trade { ..., "walletSig": <walletSig>, "walletAddress": <walletAddress> }
```
...
**⛔ DO NOT re-sign per call.** One signature per session — reuse the one you hold in memory.
**⛔ DO NOT write `walletSig` to any store, scratchpad, log, file, or transcript** — it is a bearer
credential; keep it in memory only (see FORBIDDEN).
